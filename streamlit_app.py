...
# Google Drive 파일 ID
file_id = '1mGrxGX0zcT0hil3EtW1BuvYSMAT19q2X'
...
def display_right_content(labels):
    ...
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.image(f"?text={label}", caption=f"이미지: {label}", use_column_width=True)
            st.image(f"https://i.ibb.co/wgJRScC/1.jpg?text={label}", caption=f"이미지: {label}", use_column_width=True)

    # 2nd Row - YouTube Videos based on labels
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.video("https://www.youtube.com/watch?v=IZwHcx3EzrI", start_time=0)
            st.caption(f"유튜브: {label}")

    # 3rd Row - Text based on labels
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.write(f"{label} 관련 텍스트 내용입니다. 납작당면,물냉면,비빔냉면 하하하하")
